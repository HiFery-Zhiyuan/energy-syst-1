...
用以判断各个比例的能源优化程度
Designed by Zhiyuan @ 7th, April, 2024
...
norm_gencost = 1 # 用来定义是否需要 normalize gen cost
...
norm_carbon = 1 # 用来定义是否需要 normalize gen cost
...
model.ratio_type = Set(initialize = range(0,4))              # explanation as /// 0: wind;      1: pv;       2: ES;       4: other ///
...
# 碳排最优 + 经济最优
model.obj_cost = Objective(expr =   sum(  (model.PG[i])**2*model.C[i,4]+ (model.PG[i])*model.C[i,5]+ model.C[i,6]for i in model.buses), sense = minimize)
model.obj_carbon = Objective(expr = sum( model.PG[i]*model.Carbon[i]  for i in model.buses), sense = minimize)
