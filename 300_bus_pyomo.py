...
import cmath
import numpy as np
from pypower.api import case300, ppoption, runpf, loadcase
...
model.power_supply_demand_balance.add(expr = sum(model.PG[i] for i in model.buses) - sum(model.PD[i] for i in model.buses) <= 2.5) # 存在网损 2.5 MW 的网损
